...
const PlanCard = ({ id, label, features, description }: Props) => {
  return (
    <div
      key={id}
      className="flex basis-1/4 border border-1 border-[#e3e3e3] rounded-lg flex-col mb-10 h-[600px] justify-between"
    >
      <div>
          ...
          <div className="border-y-[1px] border-[#dedede]">
            <ul className="py-5">
              {features.map((feature) => (
                <li className="flex gap-2 items-center text-gray px-2 py-1">
                  <IoIosCheckmarkCircleOutline /> {feature}
                </li>
              ))}
            </ul>
          </div>
  ...
  return (
    <div id="pricing" className="md:mt-[100px] mt-[50px]">
      <div className="flex flex-col justify-center items-center md:px-0 px-5 text-center ">
        <h2 className="md:text-[40px] text-[30px] font-semibold text-center">
          Choose your right plan!
        </h2>
        <p className="">
          Select from the best plans, ensuring your perfect needs.
        </p>
      </div>
